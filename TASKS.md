...
- [ ] **T45 — Host the API (Fly.io)**: scaffolding in place (repo-root `Dockerfile` + `.dockerignore` + `fly.toml`, app `mynichi-api`, region nrt, `TRANSLATE_BACKEND=anthropic`; lean image installs only apps/api + packages/core). Client points prod at `https://api.mynichi.app` (`PROD_API_URL` in `apps/native/src/lib/api.ts`), overridable with `EXPO_PUBLIC_API_URL`. **Remaining (needs Philip's auth):** `fly apps create mynichi-api` → `fly secrets set ANTHROPIC_API_KEY=…` → `fly deploy`, then `fly certs add api.mynichi.app` (+ CNAME) or build web with the fly.dev URL. Deploy is intended via the Fly MCP through nekko-mcp. Self-hosted SLM path stays a later option (GPU-serving open thread). · `Added: 2026-07-19`
