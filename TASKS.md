...
### Lists
- [ ] **T21 — Quick-add enrichment, cloud fallback**: API `/enrich` fills reading, meaning, example via Claude for words the offline dictionary misses (e.g. 納期, absent from the common subset); example sentences for all captures. Dictionary-first enrichment shipped in T20. · [spec](SPEC.md#practice-lists-in-progress) · `Added: 2026-07-18`
...
- [ ] **T45 — Host the API**: deploy `apps/api` (Vercel Functions or Fly) with `ANTHROPIC_API_KEY` + `TRANSLATE_BACKEND` so prod translate and practice go live; wire `EXPO_PUBLIC_API_URL` into the web build. Blocked with the GPU-serving open thread for the local SLM path. · `Added: 2026-07-19`
