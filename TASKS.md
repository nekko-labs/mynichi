...
- **Superseded by a self-trained, license-clean SLM** (2026-07-21): rather than depend on PLaMo's community license or run two models, we trained our own. `packages/slm/` distills a local Apache-2.0 teacher (Qwen3.6-27B, via LM Studio) into a LoRA fine-tune of `Qwen3-4B-Instruct-2507` (Apache-2.0) on fully synthetic "living in Japan" text, mapping Japanese directly to `{literal, practical}` JSON in one step. A hyperparameter sweep (driven by kotrain, since renamed Nekkos) picked `runs/r16-lr2e4-e3`: **score 82.43, case_pass 1.0** (all 12 tricky cases, incl. 「巻きで」 and 「持ち帰り」), chrF 56.62 vs teacher, json_valid 0.989, beating the untuned Qwen3-4B baseline (71.2 / 0.917 / 36.33). This **retires the PLaMo licensing blocker** (everything is Apache-2.0 end to end), halves the serving footprint (one 4B model vs 10B+8B), and keeps the privacy story (free-tier text stays on our infra). Served by **LM Studio** as `mynichi-translate` over its OpenAI-compatible API; `apps/api/src/translate/backends.ts` makes a single call to it and was verified 12/12 through the full `/translate` stack (kuromoji furigana/romaji + SLM). Build: `packages/slm/export_gguf.py --load` (merge LoRA -> f16 GGUF via llama.cpp b9000 -> `lms import`/`lms load`). Two serving constraints, both learned the hard way: (1) **runtime** matters, Ollama 0.23's bundled llama.cpp mis-serves this Qwen3-2507 GGUF (non-deterministic garbage, reverts 「巻きで」) while LM Studio's current runtime serves the identical file correctly, and a vLLM server is a drop-in via the same OpenAI API; (2) **decode greedily** (top_k 1, repeat_penalty 1.0) to reproduce the eval, or the sampler reverts the hard cases.
